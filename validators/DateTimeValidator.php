...
class DateTimeValidator extends Validator {
    ...
    public function init() {
        ...
        if ($this->max !== null && is_string($this->max)) {
            $timestamp = $this->parseDateTimeValue($this->max);
            if ($timestamp === false) {
    ...
    protected function parseDateTimeValue($value) {
        ...
        return $this->parseDateTimeValuePHP($value, $format);
    }
     ...
     * @return integer|boolean a UNIX timestamp or `false` on failure.
     */
    private function parseDateTimeValuePHP($value, $format) {
        // if no time was provided in the format string set time to 0 to get a simple date timestamp
    ...
    public function validateAttribute($model, $attribute) {
        ...
        $timestamp = $this->parseDateTimeValue($value);
        if ($timestamp === false) {
